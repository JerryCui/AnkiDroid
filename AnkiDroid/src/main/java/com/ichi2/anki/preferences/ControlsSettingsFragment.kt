...
class ControlsSettingsFragment :
    ...
    override fun initSubscreen() {
        ...
        setupAnswerCommands()
        setupPronunciationCheckPreference()
    }
    ...
    private fun setupPronunciationCheckPreference() {
        findPreference<SwitchPreferenceCompat>(R.string.record_voice_check_enabled_key)?.apply {
            val repository = StudyScreenRepository()
            isChecked = repository.isRecordVoiceEnabled
            setOnPreferenceChangeListener { newValue: Boolean ->
                if (newValue) {
